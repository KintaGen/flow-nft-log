...
access(all) contract PublicKintaGenNFTv2: NonFungibleToken {
    ...
    access(all) resource NFT: NonFungibleToken.NFT, ViewResolver.Resolver {
        ...
        init(projectName: String, projectSummary: String, projectCID: String, principalInvestigator: String, runHash: String) {
            self.id = PublicKintaGenNFTv2.totalSupply
        ...
        access(all) fun resolveView(_ view: Type): AnyStruct? {
            switch view {
                ...
                case Type<[PublicKintaGenNFTv2.WorkflowStepView]>():
        ...
        access(all) fun createEmptyCollection(): @{NonFungibleToken.Collection} {
            return <- PublicKintaGenNFTv2.createEmptyCollection(nftType: Type<@PublicKintaGenNFTv2.NFT>())
        }
    }
    ...
    access(all) resource Collection: NonFungibleToken.Collection, ViewResolver.ResolverCollection {
        ...
        access(all) fun deposit(token: @{NonFungibleToken.NFT}) {
            let nft <- token as! @PublicKintaGenNFTv2.NFT
            let id = nft.id
        ...
        access(all) view fun getSupportedNFTTypes(): {Type: Bool} {
            return { Type<@PublicKintaGenNFTv2.NFT>(): true }
        }

        access(all) view fun isSupportedNFTType(type: Type): Bool {
            return type == Type<@PublicKintaGenNFTv2.NFT>()
        }
    ...
    access(all) resource Minter {
        access(all) fun mint(projectName: String, projectSummary: String, projectCID: String, principalInvestigator: String, runHash: String): @NFT {
            ...
            PublicKintaGenNFTv2.totalSupply = PublicKintaGenNFTv2.totalSupply + 1
            return <- token
        }
    }
    ...
    access(all) fun borrowMinter(): &PublicKintaGenNFTv2.Minter {
        return self.account.storage.borrow<&PublicKintaGenNFTv2.Minter>(from: self.MinterStoragePath)
            ?? panic("PublicKintaGenNFTv2 minter is not stored in contract account.")
    }
        ...
        switch viewType {
            case Type<MetadataViews.NFTCollectionData>():
                return MetadataViews.NFTCollectionData(
                    ...
                    publicCollection: Type<&PublicKintaGenNFTv2.Collection>(),
                    publicLinkedType: Type<&PublicKintaGenNFTv2.Collection>(),
    ...
    init() {
        ...
        let cap = self.account.capabilities.storage.issue<&PublicKintaGenNFTv2.Collection>(self.CollectionStoragePath)
